...
CPlayerController::CPlayerController(EventHandler* i_pEventHandler) : m_pEventHandler(i_pEventHandler)
{
...
{
	if (m_uEntity == (uint32_t)-1) return;

	VelocityComp* pVelocity = m_pLevel->getComponent<VelocityComp>(m_uEntity);
